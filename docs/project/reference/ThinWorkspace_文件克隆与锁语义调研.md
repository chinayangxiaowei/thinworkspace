...
**调研日期：2026-09-12｜状态：文献核实完成；本机专项实验待执行**
...
当前 Base 仍来自 Git 提交内容，不包含来源工作树的未提交/未跟踪缓存。本文不改变这一边界；若要把既有编译缓存纳入来源，必须另行确定来源一致性和公开范围。[Phase 1 详细设计 §6](../design/ThinWorkspace_Phase1单机CLI详细设计_v1.0.md)、[ADR-0001](../architecture/adr/ADR-0001_Phase1普通目录与无执行包装.md)
...
- 若仅缓存命中失败：按工具的一致性规则诊断，不能把它归为锁隔离失败。引入缓存来源或策略需要单独的产品范围决策。
