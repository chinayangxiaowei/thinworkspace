...
| P0-06 原始目录镜像验证 | 非 Git/完整 .git/ignored 文件物化与首版 mtime 范围、源一致性边界及无 Git 控制项 | P0-02 | R4 | Verification |
| P0-07 已跟踪检查与清理提示验证 | 主/子仓库 tracked-only、Git unknown、显式强制与持久日志的最小真实验证 | P0-06 | R4 | Backlog |

P0-03 因 ADR-0002 取消，旧属性/sparse 兼容选择不再阻塞新路线；不再启动其旧全量收尾批次。P0-06/P0-07 为新任务，不复用已取消编号；P0-06 当前执行证据见下方链接，P0-07 尚未执行；P0-04 按新依赖执行。
...
当前顺序：设计文档提交 → P0-06 原始目录验证 → P0-07 tracked-only 检查 → P0-05 已排期锁实验 → P0-04 故障闭环。2026-09-13 维护者恢复项目实施，P0-06 已按流程准入并领取；基线、工作区、实际模型、构建范围处置与后续证据统一见 [P0-06 实施记录](../implementation/P0-06_原始目录镜像实验.md)。已有 P0-01/P0-02 的 Done 仅覆盖原实验声明范围，新 mtime 和完整目录需求不能直接计为通过。
