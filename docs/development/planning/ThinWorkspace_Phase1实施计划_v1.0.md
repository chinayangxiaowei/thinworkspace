...
| P0-06 原始目录镜像验证 | 非 Git/完整 .git/ignored 文件物化与首版 mtime 范围、源一致性边界及无 Git 控制项 | P0-02 | R4 | Backlog |
| P0-07 已跟踪检查与清理提示验证 | 主/子仓库 tracked-only、Git unknown、显式强制与持久日志的最小真实验证 | P0-06 | R4 | Backlog |

P0-03 因 ADR-0002 取消，旧属性/sparse 兼容选择不再阻塞新路线；不再启动其旧全量收尾批次。P0-06/P0-07 是新增、尚未执行的任务，不复用已取消编号；P0-04 按新依赖执行。
...
当前顺序：本次设计文档提交 → P0-06 原始目录验证 → P0-07 tracked-only 检查 → P0-05 已排期锁实验 → P0-04 故障闭环。各任务实施前按任务流程单独准入；本次只修改文档，不领取代码任务。已有 P0-01/P0-02 的 Done 仅覆盖原实验声明范围，新 mtime 和完整目录需求不能直接计为通过。
