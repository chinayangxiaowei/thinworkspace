...
| P0-07 已跟踪检查与清理提示验证 | 主/子仓库 tracked-only、Git unknown、显式强制与持久日志的最小真实验证 | P0-06 | R4 | In Progress |

P0-03 因 ADR-0002 取消，旧属性/sparse 兼容选择不再阻塞新路线；不再启动其旧全量收尾批次。P0-06/P0-07 为新任务，不复用已取消编号；当前执行证据见下方链接，P0-07 尚无验收结论；P0-04 按新依赖执行。
...
当前顺序：设计文档与 P0-06 已完成 → P0-07 tracked-only 检查 → P0-05 已排期锁实验 → P0-04 故障闭环。P0-06 已获规定模型正式 Approve，审核后真实 Verification、精确候选完整 CI 与代码 PR 合并均完成；基线、工作区、保真范围、CI 修复、实际模型和保留边界统一见 [P0-06 实施记录](../implementation/P0-06_原始目录镜像实验.md)的最终收口。P0-07 已由主 Agent 按流程准入并领取，实际基线、模型、范围和验收证据只在 [P0-07 实施记录](../implementation/P0-07_已跟踪检查与清理提示实验.md)维护；不扩大 P0-01/P0-02 的历史验证范围，不代表 P0.b 或整个 P0 放行。
